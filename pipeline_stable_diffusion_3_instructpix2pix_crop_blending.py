...
class StableDiffusion3PipelineOutput(BaseOutput):
    ...
    Args:
        ...
        attn_map (`Optional[List[Tuple[torch.Tensor, torch.Tensor]]]`)
            A list where each element is a pair of tensors
            containing the attention map of the target token and
            optionally any additional map (e.g., mask).
    """

    images: Union[List[PIL.Image.Image], np.ndarray]

    # 新增部分（符合你要求）
    attn_map: Optional[List[Tuple[torch.Tensor, torch.Tensor]]] = None
...
class StableDiffusion3InstructPix2PixPipeline_blending(DiffusionPipeline, SD3LoraLoaderMixin, FromSingleFileMixin):
    r"""
    Args:
    ...
    def encode_prompt(
        ...
        # print(prompt_embeds.shape)          # torch.Size([1, 154, 4096])
        # print(pooled_prompt_embeds.shape)   # torch.Size([1, 2048])
    ...
    def __call__(
        ...
        blending_mask: Optional[torch.FloatTensor] = None,
        **kwargs
    ):
        r"""
        ...
        # print(image_latents.shape)  # torch.Size([3, 16, 64, 64])
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if blending_mask is not None:
                    ...
                    t_b = timesteps[min(i+1, len(timesteps)-1)] 
                    if i < int(len(timesteps)*0.9):
                        ori_image_lantent, _, _ = image_latents.chunk(3)
                        ori_image_lantent = ori_image_lantent * self.vae.config.scaling_factor  
                        sigmas = get_sigmas(t_b.unsqueeze(0), n_dim=ori_image_lantent.ndim, dtype=ori_image_lantent.dtype)
                        noise = torch.randn_like(latents)
                        noise_ori_latents = sigmas * noise + (1.0 - sigmas) * ori_image_lantent
                        outputs_ori.append(noise_ori_latents)

                        latents = latents * blending_mask + noise_ori_latents * (1. - blending_mask)
                        outputs_blending.append(latents)
                    else:
                        outputs_blending.append(latents)
                # =====================================================================
        ...
        if output_type == "latent":
            ...
            if blending_mask is not None:
                blending_images = self.vae.decode(outputs_blending[-1] / self.vae.config.scaling_factor, return_dict=False)[0]
                blending_images = self.image_processor.postprocess(blending_images, output_type=output_type)[0]

                image = blending_images  
        ...
        if not return_dict:
            return image, all_attention_map
        
        return StableDiffusion3PipelineOutput(
            images=image,
            attn_map=all_attention_map,
        )
